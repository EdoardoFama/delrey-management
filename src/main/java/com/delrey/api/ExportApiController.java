...
public class ExportApiController {
    ...
    public void downloadCsv(HttpServletResponse response) throws IOException {
        ...
        response.setHeader("Content-Disposition", "attachment; filename=\"historico_manutencao.csv\"");

        try (PrintWriter pw = response.getWriter()) {
            pw.println("tipo,data,peca,categoria,km,valor,mao_de_obra,total,fornecedor,garantia_meses,observacoes");
            for (Troca t : trocaRepository.findByCarroIdOrderByDataTrocaDesc(carro.getId())) {
    ...
    public Map<String, Object> downloadJson(HttpServletResponse response) {
        response.setHeader("Content-Disposition", "attachment; filename=\"backup_manutencao.json\"");

        Carro carro = userContextService.getCarroDoUsuarioAtual();
        Long carroId = carro.getId();

        List<Map<String, Object>> trocas = trocaRepository.findByCarroIdOrderByDataTrocaDesc(carroId).stream()
                .map(t -> {
        ...
        List<Map<String, Object>> abastecimentos = abastecimentoRepository.findByCarroIdOrderByDataDesc(carroId).stream()
                .map(a -> {
                    Map<String, Object> m = new LinkedHashMap<>();
                    m.put("id", a.getId());
        ...
        List<Map<String, Object>> leituras = leituraKmRepository.findByCarroIdOrderByDataDesc(carroId).stream()
                .map(l -> {
                    Map<String, Object> m = new LinkedHashMap<>();
                    m.put("id", l.getId());
        ...
        List<Map<String, Object>> problemas = problemaRepository.findByCarroIdOrderByDataInicioDesc(carroId).stream()
                .map(p -> {
                    Map<String, Object> m = new LinkedHashMap<>();
                    m.put("id", p.getId());
