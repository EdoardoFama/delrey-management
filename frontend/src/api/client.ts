...
export const api = {
  ...
  getProblemas: (status?: string) => request(`/api/problemas${status ? `?status=${status}` : ''}`),
  createProblema: (body: unknown) => request('/api/problemas', { method: 'POST', body: JSON.stringify(body) }),
  updateProblema: (id: number, body: unknown) => request(`/api/problemas/${id}`, { method: 'PUT', body: JSON.stringify(body) }),
  deleteProblema: (id: number) => request(`/api/problemas/${id}`, { method: 'DELETE' }),
